...
### Environment & World Composition Architecture (v0.0.6.3)
- `EnvironmentAssetCatalog`: Standardized metadata catalog for all modular architectural and prop assets. Defines native dimensions, anchor presets (`bottom-center`, `top-left`, etc.), depth classes, physical classes (`none`, `floor-solid`, `barrier`, `overhead`), and collision profiles.
- `CollisionProfile`: Decouples visual graphic size (e.g. 73×160 massive column) from physical collision footprint (e.g. 44×22 at ground base Y - 10), preventing player clipping while allowing free movement around architectural features.
...
- `EnvironmentComposer`: Instantiates Phaser sprites from placement descriptors, calculates exact ground/sort anchors, applies semantic depth sorting, and creates Arcade physics static zone colliders.
- `WorldCompositionDebug`: Visual diagnostic tool toggled via `[V]` or `?debug=world` rendering collision footprints (green), ground sort anchors (yellow), sprite origins (cyan), and the symmetry axis (magenta).

### Lighting & Ambient FX Systems (v0.0.6.1)
...
- `AmbientFXSystem`: Manages floating dust motes in warm light cones, window rain streaks, animated fireplace hearth flames/embers, curtain breezes, and exterior silhouettes.
